    ...
    <div>
        ...
        <form onSubmit={handleSearch} style={{
          ...
          <input
            ...
            style={{
              ...
              fontSize:'1rem', width:'340px', maxWidth:'100%'
            }}
          />
      ...
      <div style={{ maxWidth:'1100px', margin:'0 auto', padding:'0 1rem' }}>
        <div style={{ display:'flex', flexWrap:'wrap', gap:'0.5rem', margin:'1.5rem 0 1rem' }}>
          ...
          {categories.map(cat => (
            <button key={cat.id} onClick={() => handleCategory(cat.id)} style={pillStyle(categoryId === cat.id)}>
              {cat.name}
            </button>
          ))}
        </div>
        ...
        {(search || categoryId > 0) && (
          <div style={{ marginBottom:'1rem', color:'#555', fontSize:'0.95rem' }}>
            ...
            {categoryId > 0 && categories.find(c => c.id === categoryId) && (
        ...
        {error && (
          <div style={{ textAlign:'center', padding:'2rem', color:'#dc2626' }}>
            ...
            <p style={{ fontSize:'0.85rem', color:'#999', marginTop:'0.5rem' }}>
              API may not be connected yet. Products will load after backend deployment.
            </p>
          </div>
        )}
...
echo "Fixing CartContext.jsx..."
cat > "$BASE/context/CartContext.jsx" << 'EOF'
